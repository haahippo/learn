...
class Conv2d(_ConvNd):
    ...
    def conv2d(self, input, kernel, bias=0, stride=1, padding=0):
        ...
        input_pad = torch.nn.functional.pad(input, (padding, padding, padding, padding))
        input_col = torch.nn.functional.unfold(input_pad, kernel_size=(KH,  KW), stride=stride)
        input_col = input_col.transpose(1, 2).reshape(-1, IC * KH * KW)
        kernel_col = kernel.reshape(OC, -1).transpose(0, 1)
        output = torch.matmul(input_col, kernel_col)
        output = output.reshape(N, OC, out_h, out_w)
        if bias is not None:
    ...
    def backward(self, ones: Tensor):
        ...
        grad_input = grad_padX[:, :, padding[0]:nPadImgRows - padding[0], padding[1]:nPadImgCols - padding[1]]
